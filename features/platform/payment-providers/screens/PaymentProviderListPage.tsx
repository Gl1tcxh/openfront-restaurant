...
export async function PaymentProviderListPage({ searchParams }: PageProps) {
  ...
  const pageSize = parseInt(searchParamsObj.pageSize?.toString() || (list as any).pageSize?.toString() || '50', 10)
  const searchString: string = searchParamsObj.search?.toString() || ''
  ...
  let statusCounts = { all: 0, installed: 0, notInstalled: 0 }
